...
consistency of the velocity of ammunition.  SD is actually a very general
idea, applied to all types of data besides projectile velocities.  
...
Here's the formal definition.  Let $v_i$ be the velocity of the $i$th
round, $\bar{v}$ be the average velocity and $n$ be the number of rounds
fired through a chronograph. Then the unbiased SD for a normal population
is given by this formula:

$$
    \sqrt{ \frac{1}{n - 1} \sum_{i = 1} (v_i - \bar{v})^2 }
$$

Another common formula uses $\frac{1}{n}$ rather than $\frac{1}{n -
1}$. This should be avoided for small samples as used here.

This formula leaves $n$ free, which means that we can estimate SD using as
...
To explore this we need to understand the idea of a [prediction
interval](https://robjhyndman.com/hyndsight/intervals/).

for the next round
from the lot.  An This is useful because one can check ballistic tables
